...
const sendItem = await Basket.create({itemname, description, image, price, refItem: id, refUser: refUser})
...
exports.deleteItemFromBasket = async (req, res, next) => {
    const {refUser} = req.params;
    const {id} = req.body;
    const deletedItem = await Basket.findOneAndDelete({ refUser: refUser, refItem: id });
